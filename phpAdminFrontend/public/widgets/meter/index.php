...
$result =  $ots->execute('module', 'get-listnew', ['table' => 'list_meterutiltype', 'field' => 'utiltype']);
...
<script>
	...
	$(document).ready(function() {
		// INIT FOR FILTER
		var list_meterutiltype = [];
		<?php foreach ($list_meterutiltype as $val) { ?>
			list_meterutiltype.push('<?= $val ?>');
		<?php } ?>
		var list_metertype = [];
		<?php foreach ($list_metertype as $val) { ?>
			list_metertype.push('<?= $val ?>');
		<?php } ?>
			...
			columns: [{
				...
				},
				{
					...
					render: function(data, row) {
						return `
						<a href="<?= WEB_ROOT . "/$module/view/" ?>${row.enc_id}" title="View [${row.meter_name}]"><i class="fa-solid fa-eye fa-lg text-info"></i></a>&emsp;
						<?php if ($role_access->update) : ?> 
						<a href="<?= WEB_ROOT . "/$module/form/" ?>${row.enc_id}" title="Edit [${row.meter_name}]"><i class="fa-solid fa-pen fa-lg text-primary"></i></a>&emsp;
						<?php endif; ?>
						...
						<a del_url="<?= WEB_ROOT . "/module/delete/$module/$table" ?>/${row.enc_id}" onclick="show_delete_modal(this)" role_access="<?= $role_access->delete ?>" 
							title="Delete [${row.meter_name}]" rec_id="${row.id}"><i class="fa-solid fa-trash fa-lg text-danger"></i></a>
							<?php endif; ?>`
					}
				},
			],
