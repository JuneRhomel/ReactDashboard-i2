...
<h3 class='h3 p-4 h3-title' >Import Permits</h3>
<hr>

<a href='<?php echo WEB_ROOT?>/sample_excel/Import Permits.xlsx' class='btn btn-link'><i class='bi bi-download text-primary' style='font-size:20px'></i> Download Permits Template</a>
<?php 
// print_r($_SESSION);
if($_GET['error']){
    ?>
...
<form action="<?php echo WEB_ROOT ?>/admin/read-excel?display=plain" class='ml-3' method="post" id="import-permits" enctype="multipart/form-data">
    <div class="form-group">
        <input type="hidden" name="table" value='permits'>
        <input type="hidden" name="view_table" value='view_permits'>
        <input type="hidden" name="update_table" value='permit_updates'> 
        <input type="hidden" name="redirect" value='<?php echo WEB_ROOT ?>/contracts/permit-tracker?submenuid=permittracker&menuid=contracts'>
        <input type="hidden" name="error_redirect" value='<?php echo WEB_ROOT ?>/admin/import-permits?submenuid=import_permit'>
...
<script>
    $(document).ready(function(){
        $("#import-permits").on('submit',function(e){
            e.preventDefault();
            $.ajax({
