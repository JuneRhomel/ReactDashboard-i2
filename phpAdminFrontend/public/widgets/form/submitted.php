...
$module = "form";  
$table = "form_uploads";  
$view = "view_forms_uploaded";
$filters = [ array('field'=>'status','label'=>'Stage','filterval'=>array('Pending','Approved','Disapproved')) ];
$fields = rawurlencode(json_encode([ "ID"=>"id","Resident Name"=>"tenant_name","Form"=>"title","Date"=>"from_unixtime(created_on) created_date","Stage"=>"status" ]));
?>
		...
		t<?=$unique_id;?> = $("#jsdata").JSDataList({
			...
			columns:[
				...
				},
				{
					...
					class: 'col-3',
					render: function(data,row){
						return '<a href="<?=WEB_ROOT;?>/form/view-submitted/' + row.id + '">' + data +'</a>';
					}
				},
				{
					data: "created_on",
				...
				},
				{
					...
					class: 'col-1',
					render: function(data,row){
						return '<a class="btn btn-sm btn-success btn-view" title="View ID ' + row.rec_id + '" href="<?=WEB_ROOT?>/<?=$module?>/view-submitted/' + row.id + '"><i class="bi bi-eye"></i></a> '+'<a class="btn btn-sm btn-danger btn-delete" title="Delete ID ' + row.rec_id + '" href="<?=WEB_ROOT?>/module/delete/<?=$module?>/<?=$table?>/' + row.id + '"><i class="bi bi-trash"></i></a>';
					},
					orderable: false
				}
			],
