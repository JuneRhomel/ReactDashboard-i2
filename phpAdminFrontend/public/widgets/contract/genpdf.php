<?php
require_once(DIR_ROOT . '/library/tcpdf/tcpdf.php');
...
$result = $ots->execute('module','get-record',[ 'id'=>$id,'view'=>'vw_contract' ]);
$record = json_decode($result);
$recarr = (array) $record;

$result = $ots->execute('module','get-record',[ 'id'=>$record->template_id,'view'=>'vw_contract_template' ]);
$contract_template = json_decode($result);

$result =  $ots->execute('module','get-listnew',[ 'table'=>'vw_contract_field','orderby'=>'id' ]);
