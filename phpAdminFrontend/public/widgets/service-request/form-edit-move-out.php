<?php
	...
	$move_out = $ots->execute('tenant','get-record',$data);
	$move_out = json_decode($move_out);
...
<div class="bg-white rounded-sm">
		<?php $contract->id?>
		<form action="<?= WEB_ROOT ?>/tenant/save-record?display=plain" method='post' id="mo-edit" class="bg-white">
			<input type="hidden" name='redirect'  id='redirect' value= '<?= WEB_ROOT?>/tenant/view-move-out/<?=$args[0]?>/View'>
			<input type="hidden" name='table'  id='id' value= 'move_out'>
			<input type="hidden" name='view_table'  id='id' value= 'view_move_out'>
			<label class="required-field mt-4">* Please Fill in the required field</label>
					...
					<div class="col-12 col-sm-4 my-4">
						...
						<label for="" class="text-required">Service Request Category</label>
							<select name="sr_type" id="sr_type" class='form-select'>
								<option value="unit-repair" <?= ($move_out->sr_type == "unit-repair")?'selected':'';?>>Unit Repair</option>
                                <option value="gate-pass"  <?= ($move_out->sr_type == "gate-pass")?'selected':'';?>>Gate Pass</option>
                                <option value="visitor-pass"  <?= ($move_out->sr_type == "visitor-pass")?'selected':'';?>>Visitor's Pass</option>
                                <option value="reservation"  <?= ($movmove_oute_in->sr_type == "reservation")?'selected':'';?>>Reservation</option>
                                <option value="move-in"  <?= ($move_out->sr_type == "move-in")?'selected':'';?>>Move-In</option>
                                <option value="move-out"  <?= ($move_out->sr_type == "move-out")?'selected':'';?>>Move-Out</option>
                                <option value="work-permit"  <?= ($move_out->sr_type == "work-permit")?'selected':'';?>>Work Permit</option>
                            </select>
						</div>
					</div>
				...
				<div class="row forms">
					<div class="col-12 col-sm-4 my-4">
						...
						<input type="date" name='date' value="<?= $move_out->date?>" min="<?= date('Y-m-d')?>"  class="form-control" required>
						</div>
					</div>
				</div>
                ...
                <div class="row">
                    <div class="col-12 col-sm-4 my-4">
						<div class="form-group">
							...
							<input name="name" type="hidden" value="<?= $move_out->name; ?>" required>
                        ...
                        <!-- <div class="form-group">
                            ...
                            <input type="text" class='form-control' name='name' value="<?= $move_out->name?>" required>
                        </div> -->
                    </div>
					...
					<div class="col-12 col-sm-4 my-4">
                        ...
                        <label for="" class="text-required">Resident Type <span class="text-danger">*</span></label>
							<select name="resident_type" id="resident_type" class='form-select'>
								<option <?= ($move_out->resident_type == "Unit Owner")?'selected':'';?>>Unit Owner</option>
								<option <?= ($move_out->resident_type == "Tenant")?'selected':'';?>>Tenant</option>
							</select>
                        </div>
                    </div>
					...
					<div class="col-12 col-sm-4 my-4">
                        ...
                        <label for="" class="text-required">Unit <span class="text-danger">*</span></label>
							<input type="text" class='form-control' name='unit' value="<?= $move_out->unit?>" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-4 my-4">
                        <div class="form-group">
                            ...
                            <input type="text" class='form-control' name='contact_num' value="<?= $move_out->contact_num?>" required>
                        </div>
                    </div>
                </div>
...
<script>
	$(document).ready(function(){
		$("#mo-edit").off('submit').on('submit',function(e){
			e.preventDefault();
			$.ajax({
