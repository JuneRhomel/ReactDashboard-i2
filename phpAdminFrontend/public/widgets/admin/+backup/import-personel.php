...
<h3 class='h3 p-4 h3-title' >Import Personnel</h3>
<hr>

<a href='<?php echo WEB_ROOT?>/sample_excel/Import Personnel.xlsx' class='btn btn-link'><i class='bi bi-download text-primary' style='font-size:20px'></i> Download Contracts Template</a>
<?php 
// print_r($_SESSION);
if($_GET['error']){
    ?>
...
<form action="<?php echo WEB_ROOT ?>/admin/read-excel?display=plain" class='ml-3' id="import-personnel" method="post" enctype="multipart/form-data">
    <!-- default table  -->
    <input type="hidden" name="table" value='building_personnel'>
    <!-- table for views -->
    <input type="hidden" name="view_table" value='building_personnel_view'>
    <!-- table for updates -->
    
    
    <input type="hidden" name="redirect" value='<?php echo WEB_ROOT ?>/property-management/building-personnel?submenuid=location&menuid=propman'>
    <input type="hidden" name="error_redirect" value='<?php echo WEB_ROOT ?>/admin/import-personel?submenuid=import_personel'>
...
<script>
    $(document).ready(function(){
        $("#import-personnel").on('submit',function(e){
            e.preventDefault();
            $.ajax({
