...
<div class="bg-white rounded-sm">
		<?php $permit->id?>
		<form action="<?= WEB_ROOT ?>/contracts/update-permit-contracts?display=plain" method='post' id="permit-add" class="bg-white">
			...
			<div class="row forms">
				 ...
				 <div class="col-12 col-sm-4 my-4">
					...
					<br>
						...
						<input type="text" name='issuing_office' class='form-control' value='<?= $permit->issuing_office?>' required>
					</div>
				</div>
...
<script>
	$(document).ready(function(){
		$("#permit-add").off('submit').on('submit',function(e){
			e.preventDefault();
			$.ajax({
				...
				success: function(data){
					if(data.success == 1)
					{
						var redirect = "<?= WEB_ROOT?>/contracts/view-permit/"+data.id+"/View";
		...
		$(".btn-cancel").off('click').on('click',function(){
			Swal.fire({
				...
				if (result.isConfirmed) {
					window.location.href = '<?php echo WEB_ROOT;?>/contracts/permit-tracker?submenuid=permittracker';
				}
			})
