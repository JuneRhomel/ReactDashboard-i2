	...
	$(document).ready(function() {
		...
		t<?= $unique_id; ?> = $("#jsdata").JSDataList({
			...
			columns: [{
					...
					// 	return '<input type="checkbox" id="' + row.id + '" name="check_box" table="contracts" view_table="view_contracts" reload="<?= WEB_ROOT; ?>/contracts/contract-tracker?submenuid=contractracker"> ' + data;
					// }
				},
				{
					data: "contract_name",
					...
					datatype: "none",
					render: function(data, row) {
						return '<a style="text-decoration: none; text-transform: capitalize; " href="<?= WEB_ROOT; ?>/contracts/view-contracts/' + row.id + '/View" target="_self">' + data + '</a>';
					}
				},
				{
				...
				// },
				{
					...
					render: function(data, row) {
						return ' <a href="<?= WEB_ROOT; ?>/module/download/?display=csv&module=<?= $module ?>&table=<?= $table ?>&view=<?= $view ?>&fields=<?= $fields ?>&id=' + row.id + '" class="btn btn-sm btn-download-icon"><i class="bi bi-download"></i><a>' +
							'<a class="btn btn-sm" onclick="show_delete_modal(this)" role_access="<?= $role_access->delete ?>" rec_id=" ' + row.rec_id + '" title="Delete ID ' + row.rec_id + '" del_url="<?= WEB_ROOT ?>/contracts/delete-permit-contracts/' + row.id + '?display=plain&table=contracts&view_table=view_contracts"><i class="bi bi-trash text-primary"></i></a>' +
		...
		$(document).on("click", ".row-id", function() {
			var id = $(this).data("id");
			window.location.href = "<?= WEB_ROOT ?>/contracts/view-contracts/" + id + "/View";
		...
		$(".btn-add").off('click').on('click', function() {
			window.location.href = "<?= WEB_ROOT . "/contracts/"; ?>form-add";
		});
