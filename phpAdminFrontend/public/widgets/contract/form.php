...
if ($contracts) {
	foreach ($contracts as $contract) {
		foreach ($units as $key => $val) {
			if ($val->id==$contract->unit_id && $val->id!=$record->unit_id)
				unset($units[$key]);
		}
	}
}
					...
					<div class="col-12 col-sm-8">
						<div class="row">
								...
								<div class="form-group input-box">
									<select name="unit_id" class="form-control form-select" required>
										...
										<?php foreach ($units as $key=>$val) { ?>
											<option value="<?=$val->id ?>" <?=($record && $record->unit_id == $val->id) ? 'selected' : '' ?> locstatus="<?=$val->status?>"><?=$val->location_name ?></option>
										<?php } ?>
									</select>
									<label>Unit <b class="text-danger">*</b></label>
								</div>
							</div>
							<div class="col-12 col-sm-6 mb-3 ownership">
								<div class="form-group input-box">
							...
							<?php if ($custom_fields) { ?>
								<h6 class="ownership"><b>CUSTOM FIELDS</b></h6>
...
<script>
	$(document).ready(function() {
		...
		// 23-0905 ATR: HIDE FIELDS IF FOR LEASE OR OCCUPIED BY LESSEE
		$("select[name=unit_id]").change(function() {
			locstatus = $('option:selected', this).attr('locstatus');			
			if (locstatus=="For Lease" || locstatus=="Occupied by Lessee")
				$(".ownership").addClass("d-none");
			else
				$(".ownership").removeClass("d-none");
console.log('locstatus',locstatus);
		});
		$("select[name=unit_id]").trigger("change");
