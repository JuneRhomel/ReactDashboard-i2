...
//$result =  $ots->execute('location','get-parent-locs',[]);
$result =  $ots->execute('module','get-listnew',[ 'table'=>'locations','field'=>'id,location_name','condition'=>'location_type!="Building"','orderby'=>'location_name' ]);
$parent_locs = json_decode($result);
//vdump($parent_locs);

$result =  $ots->execute('module','get-listnew',[ 'table'=>'list_locationtype','field'=>'locationtype' ]);
					...
					<div class="col-12 col-sm-4 mb-3 div-bldg">
						<div class="form-group input-box">
							<select name="location_type" class="form-control" required>
								...
								<option <?=($record && $record->location_type==$val->locationtype) ? 'selected':''?>><?=$val->locationtype?></option>
								<?php } ?>
							</select>
					...
					<div class="col-12 col-sm-4 mb-3 ">
						<div class="form-group input-box">
							<select name="location_use" class="form-control non-building" required>
								...
								<option <?=($record && $record->location_use==$val->locationuse) ? 'selected':''?>><?=$val->locationuse?></option>
								<?php } ?>
							</select>
					...
					<div class="col-12 col-sm-4 mb-3 ">
						<div class="form-group input-box">
							<select name="location_status" class="form-control non-building" required>
								...
								<option <?=($record && $record->location_status==$val->locationstatus) ? 'selected':''?>><?=$val->locationstatus?></option>
								<?php } ?>
							</select>
...
$(document).ready(function(){
	...
	$("#form-main").off('submit').on('submit',function(e){
		...
		$.ajax({
			...
			success: function(data){					
				if(data.success == 1) {
					toastr.success(data.description,'Information',{ timeOut:2000, onHidden: function() { location="<?=WEB_ROOT.'/'.$menu.'/'.$module?>"; }});
				}	
			},
		});
	});
	...
	$(".btn-cancel").off('click').on('click',function(){
		window.location.href = '<?=WEB_ROOT.'/'.$menu.'/'.$module?>';
		/*Swal.fire({
				title: 'CANCEL',
				text: 'The information will be not saved, do you wish to proceed?',
				icon: 'warning',
