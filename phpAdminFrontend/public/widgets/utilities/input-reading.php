...
<div class="main-container">
	...
	<?php else : ?>
		...
		<div class="d-flex justify-content-between gap-5 ">
			<!-- Electric -->
			<form action="<?= WEB_ROOT; ?>/utilities/save-record?display=plain" method="post" class="mother-meter-electric mother-meter d-flex flex-column gap-3 w-50">
				<input type="hidden" name='id' id='id_electric'>
				<input type="hidden" name='mother_meter' class='form-control' value='GL-100' readonly>

				<input type="hidden" name='redirect' id='redirect' value='<?= WEB_ROOT ?>/utilities/input-reading?menuid=utilities&submenuid=input-reading'>
				<input type="hidden" name='error_redirect' id='redirect' value='<?= WEB_ROOT ?>/utilities/input-reading?menuid=utilities&submenuid=input-reading'>
				...
				<div>
					<h3 class="d-flex justify-content-between">Electric Mother Meter: <b id="electric_mother_meter"></b> </h3>
					<h3 class="d-flex justify-content-between">Last Reading:
						<div>
							(<b id="electric_date_last"></b>)
							<b id="electric_last_reading" class="last_reading"> </b> <b> Kwhr</b>
						</div>
					</h3>
					<h3 class="d-flex justify-content-between">Last Consumption: <div> <b id="electric_last_consumption"> </b> <b>Kwhr</b></div></h3>
				</div>
				<div class="d-flex gap-3 flex-column ">
					<div class="d-flex align-items-center">
						<span class="w-50 lebal-text">Reading Date</span>
						<div class="w-50 input-box form-group ">
							<input class="w-100" id="reading_date_electric" name="date" type="date" required>
			...
			<!-- Water -->
			<form action="<?= WEB_ROOT; ?>/utilities/save-record?display=plain" method="post" class="mother-meter-water mother-meter d-flex flex-column gap-3 w-50">
				...
				<div>
					...
					<h3 class="d-flex justify-content-between">Last Consumption: <div><b id="water_last_consumption"></b> <b>CuM</b></div></h3>
				</div>
				<div class="d-flex gap-3 flex-column ">
					<div class="d-flex align-items-center">
						<span class="w-50 lebal-text">Reading Date</span>
						<div class="w-50 input-box form-group ">
							<input class="w-100" placeholder="" id="reading_date_water" name="date" type="date" required>
							<label for="">Choose date</label>
						</div>
					</div>
					<div class="d-flex align-items-center">
		...
		<div class="bg-white pb-2 px-2 pt-0 rounded">
			...
			<div class="d-flex flex-wrap align-items-center justify-content-between py-2" style="border-bottom: 1px solid">
				...
				<div class="position-relative col-12 col-sm-4 col-lg-4">
					<input type="search" class="form-control search-box" placeholder="Search" id="searchbox">
					<i class="bi bi-search position-absolute search-icon" style="right: 5px; top: 4px; font-size: 17px; color: #B4B4B4;"></i>
				</div>
			</div>
...
<script>
			...
			$.post({
				...
				success: function(data) {
					...
					if (!data.billing_data) {
						...
						$(utility_type.id).val('');
						$(utility_type.reading_date).val('');
						$(utility_type.reading_date).prop('readonly', false);
						$(utility_type.bill_amount).prop('readonly', false);
						$(utility_type.reading).prop('readonly', false);
						$(utility_type.mother_meter).text('N/A');
	...
	function load_input_reading_table(utility_type = 'all') {
		...
		$.post({
			url: "<?php WEB_ROOT ?>/utilities/new-input-reading?display=plain",
			data: {
