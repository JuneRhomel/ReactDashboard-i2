...
<h3 class='h3 p-4 h3-title' >Import Service Providers</h3>
<hr>
<a href='<?php echo WEB_ROOT?>/sample_excel/Import Service Provider.xlsx' class='btn btn-link'><i class='bi bi-download text-primary' style='font-size:20px'></i> Download Contracts Template</a>
<?php 
// print_r($_SESSION);
if($_GET['error']){
    ?>
...
<form action="<?php echo WEB_ROOT ?>/admin/read-excel?display=plain" class='ml-3' method="post" id="import-serviceproviders" enctype="multipart/form-data">
    <!-- default table  -->
    <input type="hidden" name="table" value='service_providers'>
    <!-- table for views -->
    <input type="hidden" name="view_table" value='service_providers_view'>
    <!-- table for updates -->
    
    
    <input type="hidden" name="redirect" value='<?php echo WEB_ROOT ?>/property-management/serviceprovider?submenuid=serviceproviders&menuid=propman'>
    <input type="hidden" name="error_redirect" value='<?php echo WEB_ROOT ?>/admin/import-service-providers?submenuid=import_service_provider'>
...
<script>
    $(document).ready(function(){
        $("#import-serviceproviders").on('submit',function(e){
            e.preventDefault();
            $.ajax({
