...
<script>
	...
	$(document).ready(function() {
		$('#form-renew').submit(function(e) {
			...
			e.preventDefault();
			$.post({
				url: '<?= WEB_ROOT ?>/permit-tracker/renew-permits?display=plain',
				data: data,
		...
		t<?= $unique_id; ?> = $("#jsdata").JSDataList({
			...
			rowLink: {
				url: '<?= WEB_ROOT; ?>/permit-tracker/view-permit/',
				params: [{
					"key": "id",
					"value": "encryptedid"
				}],
			...
			columns: [{
					...
					// 	return '<input type="checkbox" id="' + row.id + '" name="check_box" table="permits" view_table="view_permits" reload="<?= WEB_ROOT; ?>/permit-tracker/permit-tracker?submenuid=permittracker"> ' + data;
					// }
				},
				{
					data: "permit_name",
					...
					datatype: "none",
					render: function(data, row) {
						return '<a href="<?= WEB_ROOT; ?>/permit-tracker/view-permit/' + row.id + '/View" target="_self">' + data + '</a>';
					}
				},
				{
					data: "permit_number",
				...
				// },
				{
					...
					render: function(data, row) {
						return ' <a href="<?= WEB_ROOT; ?>/module/download/?display=csv&module=<?= $module ?>&table=<?= $table ?>&view=<?= $view ?>&fields=<?= $fields ?>&id=' + row.id + '" class="btn btn-sm btn-download-icon"><i class="bi bi-download text-primary"></i><a>' +
							'<a class="btn btn-sm text-primary btn-delete" role_access="<?= $role_access->delete ?>" onclick="show_delete_modal(this)" rec_id=" ' + row.rec_id + '" title="Delete ID ' + row.rec_id + '" del_url="<?= WEB_ROOT ?>/permit-tracker/delete-permit-contracts/' + row.id + '?display=plain&table=permits&view_table=view_permits"><i class="bi bi-trash"></i></a>' +
		...
		$(document).on("click", ".row-id", function() {
			var id = $(this).data("id");
			window.location.href = "<?= WEB_ROOT ?>/permit-tracker/view-permit/" + id + "/View";
		...
		$(".btn-add").off('click').on('click', function() {
			window.location.href = "<?= WEB_ROOT . "/permit-tracker/"; ?>form-add-permit";
		});
