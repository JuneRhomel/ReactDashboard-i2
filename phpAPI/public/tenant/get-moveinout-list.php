...
try{
	$return_value = $db->getRecords("{$account_db}.view_tenant_movements","id",$data,null,(isset($data['filter']) ? $data['filter'] : null));
	...
	foreach($return_value['data'] as $index=>$record)
	{
		...
		$return_value['data'][$index]['enc_owner_id'] = encryptData($record['owner_id']);
