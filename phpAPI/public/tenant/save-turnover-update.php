...
try{
	...
	$data['turnover_id'] = decryptData($data['turnover_id']);
	...
	//update ticket
	$sth = $db->prepare("update {$account_db}.turnovers set status=?,closed=? where id=?");
	$sth->execute([$data['status'],($data['status'] == 'Completed' ? 1 : 0),$data['turnover_id']]);
