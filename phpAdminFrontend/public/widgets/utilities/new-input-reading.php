    ...
    <tbody class="table-body">
        ...
        ?>
            ...
            <tr class="tr-data reading-table">
                ...
                <?php } ?>
                <td>
                    <input value="<?= $current_reading->reading ?? '' ?>" <?= ($current_reading->reading == 0) ? '' : 'disabled' ?> name="meter_reading[]" id="meter_reading" type="number" onkeypress="return check(event,value)" oninput="checkLength()" class="input_0  meter_reading" placeholder="-" required>
        ...
        <tr class="tr-data">
            ...
            <td></td>
            <td>
                <input type="hidden" class='total_con<?= $ctr ?>' value='<?= ($total_con) ?>' name='total_con[]'>
                <span class='span-total_con<?= $ctr ?>'><?= $total_con ?></span>
            </td>
            <td></td>
            <!-- <td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.tr-data').each(function() {
            ...
            const new_consumption_number = meterReadingInput.val() - last_consumption.text()
            ...
            meterReadingInput.on('input', function() {
                const new_consumption_number = $(this).val() - last_consumption.text()
                new_consumption.text(new_consumption_number)
                input_consumption.val(new_consumption_number)
            })
            ...
            const sendFormData = function(formData) {
                $.ajax({
                    url: '<?= WEB_ROOT; ?>/utilities/input-new-reading?display=plain',
                    dataType: 'json',
                    type: 'post',
                    data: formData,
                    processData: false,
        ...
        //         formData.append('new_reading', meter_reading); 
        //         formData.append('consumption', consumption); 
        //         upload.val()? formData.append('image', image) : ''
        //     //   console.log(image )
