...
$fields = rawurlencode(json_encode(["ID"=>"id","Company"=>"company","Contract Person"=>"contact_person","Email"=>"email","Contract Number"=>"contact_number","Scope of Service"=>"scope_of_service"]));
...
<div class="modal" tabindex="-1" role="dialog" id='renew'>
	<div class="modal-dialog  modal-dialog-centered" role="document">
		...
		$('.btn-import').on('click', function(){
			window.location.href = '<?=WEB_ROOT;?>/admin/import-service-providers?submenuid=import_service_provider';
		});
		...
		t<?=$unique_id;?> = $("#jsdata").JSDataList({
			...
			rowLink: {
				url: '<?=WEB_ROOT;?>/property-management/view-service-provider/',
				params : [
					{
						"key":"id",
						"value": "encryptedid"
			...
			buttons: [
				{
					...
					id: "edit",
					href: "<?=WEB_ROOT?>/property-management/form-add-serviceprovider",
				},
				//
			...
			columns:[
				{
					...
					// 	return '<input class="" type="checkbox" id="'+ row.id +'" name="check_box" table="service_providers" view_table="service_providers_view"  reload="<?=WEB_ROOT;?>/property-management/serviceprovider?submenuid=serviceproviders">'+
					// 	data;
				...
				},
				{
					...
					datatype: "none",
					render: function(data,row){
						return '<a style="text-decoration: none;" href="<?=WEB_ROOT;?>/property-management/view-service-provider/' + row.id + '/View" target="_self">' + data +'</a>';
					}
				},
				{
					data: "contact_person",
				...
				// },
				{
					...
					datatype: "none",
					render: function(data,row){
						return data.split("/").map(i => '<a class="link" href="mailto:'+ i +'">' + i + '</a>')
				...
				},
				{
					...
					class: '',
					datatype: 'select',
					list: ["0|<span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span>",
							"1| <span sclass='star-filter'>&#x2605</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span>",
							"2|<span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span>",
							"3| <span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span class='star-filter'>&#x2606</span><span class='star-filter'>&#x2606</span>",
							"4|<span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span class='star-filter'>&#x2606</span>",
							"5|<span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span><span sclass='star-filter'>&#x2605</span>",],
					render: function(data,row){
							...
							for($i = 1; $i <= 5; $i++){
								...
								if($stars >= $count){
									$result += "<span class='star' font-size: 20px;'>&#x2605</span>";
								} else {
									$result += "<span class='star-filter'>&#x2606</span>";
								}
								$count++;
							}
				...
				// 		'<a class="btn btn-sm btn-success btn-view" title="View ID ' + row.rec_id + '" href="<?=WEB_ROOT?>/property-management/view-service-provider/' + row.id + '/View"><i class="bi bi-eye"></i></a> '+
				// 		'<a class="btn btn-sm btn-danger btn-delete" onclick="show_delete_modal(this)" title="Delete ID ' + row.rec_id + '" del_url="<?=WEB_ROOT?>/property-management/delete-record/' + row.id + '?display=plain&table=service_providers&view_table=service_providers_view&redirect=/property-management/serviceprovider?submenuid=serviceproviders"><i class="bi bi-trash"></i></a>';
				// 	},
				// 	orderable: false
				// }
				{
					data: null,
					label: "Action",
					class: 'col-xl-1 col-md-6  text-center',
					render: function(data,row){
						return '<a class="btn btn-sm text-primary btn-delete-icon" onclick="show_delete_modal(this)"  role_access="<?=$role_access->delete ?>" title="Are you sure?" rec_id="'+row.rec_id+'" del_url="<?=WEB_ROOT?>/property-management/delete-record/' + row.id + '?display=plain&table=service_providers&view_table=service_providers_view&redirect=/property-management/serviceprovider?submenuid=serviceproviders"><i class="bi bi-trash-fill"></i></a>'
						// '<a class="btn btn-sm text-primary btn-delete" onclick="show_delete_modal(this)" title="Delete ID ' + row.rec_id + '" del_url="<?=WEB_ROOT?>/tenant/delete-record/' + row.id + '?display=plain&table=tenant&view_table=view_tenant&redirect=/tenant/tenant-list?submenuid=tenant_list"><i class="bi bi-trash-fill"></i></a>';
		...
		$(document).on("click", ".row-id" , function () {
			...
			var id = $(this).data("id");
			window.location.href="<?=WEB_ROOT?>/property-management/view-service-provider/" + id + "/View";
			// console.log(data);
			// alert('div clicked')
		});
