...
try{
	...
	if ($table=="occupant_reg") {
		...
		if ($status=="Approved") {		
			...
			$sql = "insert into {$account_db}.resident (email,first_name,last_name,type,unit_id,def_unit_id,address,contact_no,created_by,created_on) 
				select email,first_name,last_name,type,unit_id,def_unit_id,address,contact_no,created_by,".time()." from {$account_db}.occupant_reg where id=?";
			$sth = $db->prepare($sql);	
			$sth->execute([ $id ]);
