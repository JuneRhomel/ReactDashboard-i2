...
try{
	...
	$duplicate =  $data['duplicate_check'] ?? null;
	unset($data['duplicate_check']);
    unset($data['table']);
	...
	if($id) {
		if($duplicate != NULL){
			...
			$sql1 = "select *  from {$account_db}.{$table} WHERE $duplicate = :value and id <> $id";
		...
		if($duplicate != NULL){
			//checking duplicate entry
			$sql1 = "select *  from {$account_db}.{$table}  WHERE $duplicate = :value";
			$record_sth1 = $db->prepare($sql1);
			$record_sth1->execute([
				'value'=>$data["{$duplicate}"]
			]);
