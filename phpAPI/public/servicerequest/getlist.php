...
try{
	...
	$limit  = $data['limit'] ?? 5;
	$records = $db->prepare("select servicerequests.*,locations.location_name from {$account_db}.servicerequests left join  {$account_db}.locations on locations.id=servicerequests.location_id where tenant_id=? limit {$offset},{$limit}");
